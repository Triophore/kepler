...
console.log("Moving Sockets files.. to build directory");
copydir.sync(Path.join(__dirname,"..", 'sockets'),Path.join(__dirname,"..", options.outDir,'sockets'), {
  utimes: true,  // keep add time and modify time
  mode: true,    // keep file mode
  cover: true    // cover file when exists, default is true
});

console.log("Moving Validator files.. to build directory");
copydir.sync(Path.join(__dirname,"..", 'validators'),Path.join(__dirname,"..", options.outDir,'validators'), {
  utimes: true,  // keep add time and modify time
  mode: true,    // keep file mode
  cover: true    // cover file when exists, default is true
});

console.log("Moving Auth files.. to build directory");
copydir.sync(Path.join(__dirname,"..", 'auth'),Path.join(__dirname,"..", options.outDir,'auth'), {
  utimes: true,  // keep add time and modify time
  mode: true,    // keep file mode
  cover: true    // cover file when exists, default is true
});
...
delete get_package.scripts.build;
delete get_package.scripts.dev;
delete get_package.scripts.docs;
delete get_package.scripts.gen;

get_package.scripts.start = "node index.js";
